...
TargetVerifierBatchFixedBenchmark benchmark_target_verifier_batch_fixed(
  ...
  const bool advance = std::getenv("G4_ADVANCE_FIXED") != nullptr;
  ...
  const char* short_context_env = std::getenv("G4_FIXED_SHORT_CONTEXT");
  ...
  if (short_context < 1 || short_context > context_tokens)
    throw std::runtime_error("G4_FIXED_SHORT_CONTEXT must be in [1, CONTEXT]");
  ...
  const char* fixed_tile_env = std::getenv("G4_FIXED_FP8_TILE_ROWS");
  ...
  launch();
  const bool profile = std::getenv("G4_PROFILE_FIXED") != nullptr;
