...
struct Fp8LinearRunner::Impl {
  ...
  Impl(const DeviceModel& exact_model, const DeviceModel& fp8_model,
       ...
       int workspace_count, bool vision)
    ...
    if (!vision && !assistant &&
        (std::getenv("G4_FUSED_TARGET_GATE_UP") || std::getenv("G4_PREPARE_FUSED_TARGET_GATE_UP"))) {
      for (int layer = 0; layer < 30; ++layer) {
    ...
    check(cudaMalloc(&workspace, workspace_bytes),
          "cudaMalloc(FP8 cuBLASLt workspace)");
    if (concurrent_workspaces >= 2)
      check(cudaMalloc(&auxiliary_workspace, workspace_bytes),
            "cudaMalloc(auxiliary FP8 cuBLASLt workspace)");
    if (concurrent_workspaces >= 3)
      check(cudaMalloc(&tertiary_workspace, workspace_bytes),
            "cudaMalloc(tertiary FP8 cuBLASLt workspace)");
    ...
    if (cublasLtCreate(&lt_handle) != CUBLAS_STATUS_SUCCESS)
      throw std::runtime_error("cublasLtCreate(FP8) failed");
  ...
  ~Impl() {
    ...
    if (tertiary_workspace) cudaFree(tertiary_workspace);
    if (auxiliary_workspace) cudaFree(auxiliary_workspace);
  ...
  bool launch(const void* exact_weight, int outputs, int inputs,
    ...
    if (!weight.initialized) {
      ...
      const char* algorithm_start_env = std::getenv("G4_FP8_ALGO_START");
      int algorithm_start =
      ...
      if (!algorithm_start_env && padded_tokens == 64 &&
          std::getenv("G4_TUNED_B8_PROJECTIONS")) {
      ...
      const bool tune = tokens <= padded_tokens &&
          ((padded_tokens <= 64 && std::getenv("G4_TUNE_FP8_PROJECTIONS")) ||
           (padded_tokens > 64 && std::getenv("G4_TUNE_FP8_PREFILL")));
