...
VisionEncoderBenchmark benchmark_vision_encoder(
    ...
    const VisionFusedWeights* fused_weights) {
  ...
  const bool fused_vision_attention =
      std::getenv("G4_LEGACY_VISION_ATTENTION") == nullptr;
  ...
  const std::size_t upload_pixel_count = split_pair_input
  ...
  auto* d_pixels = static_cast<float*>(allocate_bytes(
      upload_pixel_count * sizeof(float), "cudaMalloc(vision encoder pixels)"));
  auto* d_positions = static_cast<std::int32_t*>(allocate_bytes(
  ...
  const bool native_cudnn_layout = fused_vision_attention &&
      std::getenv("G4_CUDNN_PACKED_LAYOUT") == nullptr;
  const bool fused_projections = fused_weights && !fp8_linears &&
      native_cudnn_layout &&
      std::getenv("G4_DISABLE_FUSED_VISION_PROJECTIONS") == nullptr;
  ...
  auto launch = [&] {
    const char* first_layer_env = std::getenv("G4_VISION_FP8_FIRST_LAYER");
    const int fp8_first_layer = first_layer_env
        ? std::clamp(std::atoi(first_layer_env), 0, 27) : 0;
    const char* fp8_mask_env = std::getenv("G4_VISION_FP8_MASK");
    ...
    for (int layer = 0; layer < 27; ++layer) {
      ...
      if (fused_vision_attention) {
        const char* scale_env = std::getenv("G4_CUDNN_VISION_SCALE");
  ...
  } else if (context && std::getenv("G4_VISION_GRAPHS") != nullptr) {
    const std::uint64_t graph_key = 0x5649534900000000ULL ^
        (static_cast<std::uint64_t>(kComputePatches) << 32) ^
        (reinterpret_cast<std::uintptr_t>(d_pixels) >> 8) ^
        (reinterpret_cast<std::uintptr_t>(projected) >> 8);
    if (context->has_graph(graph_key)) {
      microseconds = event_benchmark(
          stream, [&] { context->launch_graph(graph_key); }, 0, 1);
    } else {
      microseconds = event_benchmark(stream, launch, 0, 1);
      context->capture_graph(graph_key, launch);
    }
  } else {
    microseconds = event_benchmark(stream, launch, 0, 1);
  }
