...
struct KvCache::VirtualRegion {
  ...
  ~VirtualRegion() {
    if (mapped_bytes) cuMemUnmap(address, mapped_bytes);
  ...
  void map(std::size_t target, const CUmemAllocationProp& properties,
    ...
    if (target <= mapped_bytes) return;
    const std::size_t addition = target - mapped_bytes;
    CUmemGenericAllocationHandle handle{};
    check_driver(cuMemCreate(&handle, addition, &properties, 0),
                 "cuMemCreate(KV)");
    try {
      check_driver(cuMemMap(address + mapped_bytes, addition, 0, handle, 0),
                   "cuMemMap(KV)");
      CUmemAccessDesc access{};
      access.location = properties.location;
      access.flags = CU_MEM_ACCESS_FLAGS_PROT_READWRITE;
      check_driver(cuMemSetAccess(address + mapped_bytes, addition, &access, 1),
                   "cuMemSetAccess(KV)");
      const auto first_new_byte = address + mapped_bytes;
      mapped_bytes = target;
      // Direct ragged GEMMs read masked columns beyond a short row's logical
      // length. Those addresses must be both mapped and finite (0 * NaN in PV
      // is still NaN). Clear only newly committed pages, never live KV data.
      check_driver(cuMemsetD8Async(first_new_byte, 0, addition, stream),
                   "zero newly committed KV pages");
      if (!stream)
        check_driver(cuStreamSynchronize(nullptr), "initialize KV pages");
    } catch (...) {
      cuMemRelease(handle);
      throw;
    }
    check_driver(cuMemRelease(handle), "cuMemRelease(KV handle)");
  }
...
DeviceModel::DeviceModel(const std::filesystem::path& model_directory)
    : weights_(model_directory), arena_(weights_.shard_data()) {}
...
RuntimeWeights::RuntimeWeights(const std::filesystem::path& model_directory,
    ...
    : model_(model_directory), experts_(expert_directory) {
...
AssistantWeights::AssistantWeights(const std::filesystem::path& model_directory)
  ...
  if (std::getenv("G4_DISABLE_ASSISTANT_FUSED_GATE_UP") == nullptr) {
...
void* DeviceScratchPool::allocate(std::size_t bytes) {
  ...
  if (cursor_ == blocks_.size()) {
    ...
    const std::size_t capacity =
        std::max(bytes, blocks_[cursor_].bytes * 2);
