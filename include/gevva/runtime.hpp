...
class DeviceModel {
 public:
  explicit DeviceModel(const std::filesystem::path& model_directory,
                       bool omit_original_experts = false);
 ...
 private:
  ...
  bool selective_upload_{};
  std::unordered_map<std::string, std::size_t> tensor_spans_;
  DeviceArena arena_;
};
...
class KvCache {
 public:
  ...
  // Research branch cache: share complete immutable global-KV physical pages,
  // copy the boundary page and keep sliding rings/private suffixes independent.
  // Destination must be fresh. Source becomes sealed against further prefill.
  void share_global_prefix_from(KvCache& source, int prefix_tokens);
  int shared_prefix_tokens() const { return shared_prefix_tokens_; }
  std::size_t shared_global_bytes() const { return shared_global_bytes_; }
 ...
 private:
  ...
  int shared_prefix_tokens_{};
  std::size_t shared_global_bytes_{};
  bool sealed_{};
