...
gevva::RgbImage decode_jpeg(jpeg_decompress_struct& decoder) {
  jpeg_read_header(&decoder, TRUE);
  decoder.out_color_space = JCS_RGB;
  jpeg_start_decompress(&decoder);
  gevva::RgbImage result;
...
gevva::RgbImage load_jpeg(const std::filesystem::path& path) {
...
gevva::RgbImage load_png(const std::filesystem::path& path) {
  png_image image{};
...
gevva::RgbImage load_jpeg(std::span<const std::uint8_t> encoded) {
  jpeg_decompress_struct decoder{};
  JpegError error{};
  decoder.err = jpeg_std_error(&error.base);
  error.base.error_exit = jpeg_error_exit;
...
gevva::RgbImage load_png(std::span<const std::uint8_t> encoded) {
  png_image image{};
  image.version = PNG_IMAGE_VERSION;
...
std::vector<float> resize_bicubic_antialias_patches(
    const gevva::RgbImage& image, int width, int height, int worker_threads) {
...
ResizeAxis gemma4_resize_axis(int source, int destination) {
  const auto table = resize_contributions(source, destination);
  ResizeAxis result;
  result.precision = table.precision;
  for (const auto& row : table.rows) {
    result.entries.push_back({row.first, static_cast<int>(result.weights.size()),
                              static_cast<int>(row.weights.size())});
    result.weights.insert(result.weights.end(), row.weights.begin(), row.weights.end());
  }
  return result;
}

VisionInput gemma4_image_geometry(int source_width, int source_height, int maximum_soft_tokens) {
  constexpr int kPatch = 16, kPool = 3;
  if (source_width < 1 || source_height < 1)
    throw std::runtime_error("invalid RGB image geometry");
  if (maximum_soft_tokens != 70 && maximum_soft_tokens != 140 && maximum_soft_tokens != 280 &&
      maximum_soft_tokens != 560 && maximum_soft_tokens != 1120)
    throw std::runtime_error("invalid image token budget");
  ...
  int height = static_cast<int>(std::floor(source_height * factor /
                                           kSideMultiple)) * kSideMultiple;
  int width = static_cast<int>(std::floor(source_width * factor /
                                          kSideMultiple)) * kSideMultiple;
  ...
  if (height == 0) {
    ...
    width = std::min((source_width / source_height) * kSideMultiple, maximum_side);
  } else if (width == 0) {
    width = kSideMultiple;
    height = std::min((source_height / source_width) * kSideMultiple, maximum_side);
  }
  ...
  result.position_ids.resize(static_cast<std::size_t>(patches) * 2);
  for (int i = 0; i < patches; ++i) {
    result.position_ids[i * 2] = i % patch_width;
    result.position_ids[i * 2 + 1] = i / patch_width;
  }
  return result;
}
