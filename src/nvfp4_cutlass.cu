...
namespace gevva_cutlass_detail {
...
class DynamicExpertPlan {
 public:
  DynamicExpertPlan(int output_width, int input_width, int maximum_rows,
    ...
    if (std::getenv("GEVVA_PREPARE_EXPERT_TILES")) {
  ...
  void run(cudaStream_t stream) {
    ...
    if (cooperative_ && std::getenv("GEVVA_EXPERT_COOPERATIVE")) {
      cutlass_check(cooperative_->run(stream), "CUTLASS cooperative grouped experts run");
      return;
    }
    if (wide_ && std::getenv("GEVVA_EXPERT_WIDE_TILES")) {
      cutlass_check(wide_->run(stream), "CUTLASS wide grouped experts run");
      return;
    }
    if (narrow_ && std::getenv("GEVVA_EXPERT_NARROW_TILES")) {
...
using namespace gevva_cutlass_detail;
...
struct Nvfp4VocabRunner::Impl {
  ...
  Impl(const DeviceModel& exact_model, const std::string& exact_tensor,
      ...
      : maximum_tokens(tokens),
        ...
        single_problem(
            std::getenv("GEVVA_NVFP4_VOCAB_SINGLE_PROBLEM") != nullptr ||
            (!host_token_map.empty() &&
             std::getenv("GEVVA_DISABLE_HOT_NVFP4_VOCAB_SINGLE_PROBLEM") ==
                 nullptr)),
    ...
    const char* candidate_env = exact_tensor == "model.embed_tokens.weight"
        ? std::getenv("GEVVA_ASSISTANT_NVFP4_VOCAB_CANDIDATES_PER_TILE")
        : nullptr;
    if (!candidate_env)
      candidate_env = std::getenv("GEVVA_NVFP4_VOCAB_CANDIDATES_PER_TILE");
    if (const char* value = candidate_env) {
      ...
      if (candidates_per_tile != 1 && candidates_per_tile != 2 &&
        ...
        throw std::runtime_error(
            "GEVVA_NVFP4_VOCAB_CANDIDATES_PER_TILE must be 1, 2, or 4");
    }
  ...
  void launch(const __nv_bfloat16* activations, int* selected, int tokens,
    ...
    const bool grouped_top = candidates_per_tile <= 2 &&
        std::getenv("GEVVA_DISABLE_NVFP4_VOCAB_GROUPED_TOP") == nullptr;
    if (hot_vocab) {
      ...
      if (std::getenv("GEVVA_DISABLE_NVFP4_VOCAB_REDUCTION_TOP") == nullptr) {
        if (candidates_per_tile == 1)
          local_top_vocab_rows_reduce_2048<2>
...
struct ExpertScratch {
  ...
  explicit ExpertScratch(int tokens)
    ...
    if (tokens < 1 || tokens > 4608) throw std::runtime_error("invalid expert scratch capacity");
...
struct Nvfp4ExpertRunner::Impl {
  int maximum_tokens{};
  int routes{};
  std::shared_ptr<ExpertScratch> scratch;
  decltype(ExpertScratch::input)& input;
  decltype(ExpertScratch::input_scales)& input_scales;
  decltype(ExpertScratch::w13_output)& w13_output;
  decltype(ExpertScratch::intermediate)& intermediate;
  decltype(ExpertScratch::intermediate_scales)& intermediate_scales;
  decltype(ExpertScratch::w2_output)& w2_output;
  decltype(ExpertScratch::sorted_expert_ids)& sorted_expert_ids;
  decltype(ExpertScratch::route_indices)& route_indices;
  decltype(ExpertScratch::sorted_route_indices)& sorted_route_indices;
  decltype(ExpertScratch::inverse_routes)& inverse_routes;
  decltype(ExpertScratch::expert_offsets)& expert_offsets;
  decltype(ExpertScratch::scale_row_offsets)& scale_row_offsets;
  decltype(ExpertScratch::sort_workspace)& sort_workspace;
  decltype(ExpertScratch::sort_workspace_bytes)& sort_workspace_bytes;
  ...
  Impl(const DeviceModel& experts, int layer, int tokens, std::shared_ptr<ExpertScratch> shared)
      : maximum_tokens(tokens), routes(tokens * 8),
        scratch(shared ? std::move(shared) : std::make_shared<ExpertScratch>(tokens)),
        input(scratch->input),
        input_scales(scratch->input_scales),
        w13_output(scratch->w13_output),
        intermediate(scratch->intermediate),
        intermediate_scales(scratch->intermediate_scales),
        w2_output(scratch->w2_output),
        sorted_expert_ids(scratch->sorted_expert_ids),
        route_indices(scratch->route_indices),
        sorted_route_indices(scratch->sorted_route_indices),
        inverse_routes(scratch->inverse_routes),
        expert_offsets(scratch->expert_offsets),
        scale_row_offsets(scratch->scale_row_offsets),
        sort_workspace(scratch->sort_workspace),
        sort_workspace_bytes(scratch->sort_workspace_bytes),
        ...
        fused_route_sort(tokens >= 16 &&
                         std::getenv("GEVVA_DISABLE_FUSED_ROUTE_SORT") == nullptr) {
    if (scratch->maximum_tokens != tokens) throw std::runtime_error("shared expert capacity mismatch");
...
Nvfp4ExpertRunner::Nvfp4ExpertRunner(const DeviceModel& experts, int layer,
                                     int maximum_tokens, Nvfp4ExpertRunner* shared_scratch)
    : impl_(std::make_unique<Impl>(experts, layer, maximum_tokens,
          shared_scratch ? shared_scratch->impl_->scratch : nullptr)) {}
...
ExpertRepeatTestResult test_nvfp4_expert_repeatability(
  ...
  const bool identical_rows =
      std::getenv("GEVVA_EXPERT_IDENTICAL_ROWS") != nullptr;
  ...
  const bool compare_baseline = std::getenv("GEVVA_EXPERT_COMPARE_BASELINE") != nullptr;
  const bool was_wide = std::getenv("GEVVA_EXPERT_WIDE_TILES") != nullptr;
  const bool was_narrow = std::getenv("GEVVA_EXPERT_NARROW_TILES") != nullptr;
  const bool was_cooperative = std::getenv("GEVVA_EXPERT_COOPERATIVE") != nullptr;
  if (compare_baseline) {
    unsetenv("GEVVA_EXPERT_WIDE_TILES");
    unsetenv("GEVVA_EXPERT_NARROW_TILES");
    unsetenv("GEVVA_EXPERT_COOPERATIVE");
  }
  launch();
  ...
  if (compare_baseline) {
    if (was_wide) setenv("GEVVA_EXPERT_WIDE_TILES", "1", 1);
    if (was_narrow) setenv("GEVVA_EXPERT_NARROW_TILES", "1", 1);
    if (was_cooperative) setenv("GEVVA_EXPERT_COOPERATIVE", "1", 1);
  }
