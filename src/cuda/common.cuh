...
bool exact_attention_opt_enabled() {
  const char* setting = std::getenv("GEVVA_EXACT_ATTENTION_OPT");
  return !setting || std::strcmp(setting, "0") != 0;
}

std::uint64_t experimental_expert_graph_key() {
  if (!std::getenv("GEVVA_PREPARE_EXPERT_TILES")) return 0;
  if (std::getenv("GEVVA_EXPERT_COOPERATIVE")) return 0x0030000000000000ULL;
  if (std::getenv("GEVVA_EXPERT_WIDE_TILES")) return 0x0010000000000000ULL;
  if (std::getenv("GEVVA_EXPERT_NARROW_TILES")) return 0x0020000000000000ULL;
  return 0;
}
...
Bf16TargetLayerWeights target_layer_weights(
    const gevva::RuntimeWeights& weights, int layer) {
