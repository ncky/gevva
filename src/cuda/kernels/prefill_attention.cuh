...
void launch_prefill_attention_impl(
  ...
  if (experimental_text && !global) {
    ...
    if (gevva::cudnn_text_plan_available(1, query_tokens, visible_keys)) {
      gevva::launch_cudnn_text_prefill(query, key, value, output, 1,
                                   query_tokens, stream, visible_keys);
      return;
    }
  }
  if (std::getenv("GEVVA_DISABLE_BATCHED_PREFILL_ATTENTION")) {
