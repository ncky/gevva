...
TargetVerifierBatchResult launch_target_verifier_batch(
  ...
  const int add_norm_threads =
      fused_norm_threads("GEVVA_ATTENTION_NORM_THREADS");
  const int router_norm_threads =
      fused_norm_threads("GEVVA_ROUTER_NORM_THREADS");
  const int final_norm_threads =
      fused_norm_threads("GEVVA_FINAL_NORM_THREADS");
  const char* attention_router_setting = std::getenv("GEVVA_FUSE_ATTENTION_ROUTER");
  ...
  const char* gemm_attention_rows_env =
      std::getenv("GEVVA_GEMM_ATTENTION_MAX_CONTEXT_ROWS");
  ...
  const bool gemm_attention =
      std::getenv("GEVVA_SCALAR_ATTENTION") == nullptr &&
  ...
  const bool graph_decode = gemm_attention &&
      std::getenv("GEVVA_DECODE_GRAPHS") != nullptr;
  ...
  const bool tiled_attention = gemm_attention && (sessions == 1 || sessions == 8) &&
      std::getenv("GEVVA_STAGED_ATTENTION") != nullptr;
  const bool exact_attention_opt = exact_attention_opt_enabled();
  const bool fast_gemm_softmax = exact_attention_opt || std::getenv("GEVVA_FAST_GEMM_SOFTMAX") != nullptr;
  const bool fused_attention_unpack = gemm_attention &&
      (exact_attention_opt || std::getenv("GEVVA_FUSED_ATTENTION_UNPACK") != nullptr);
  ...
  bool direct_global_attention = gemm_attention && (sessions >= 4 || tiled_global) &&
      std::getenv("GEVVA_DISABLE_DIRECT_GLOBAL_ATTENTION") == nullptr;
  const bool direct_sliding_attention = gemm_attention &&
      std::getenv("GEVVA_DISABLE_DIRECT_SLIDING_ATTENTION") == nullptr;
  const bool fused_qkv_norm =
      std::getenv("GEVVA_DISABLE_FUSED_QKV_NORM") == nullptr;
  const bool combined_qkv_transform =
      std::getenv("GEVVA_DISABLE_COMBINED_QKV_TRANSFORM") == nullptr;
  const bool fused_projection_norms =
      std::getenv("GEVVA_DISABLE_FUSED_PROJECTION_NORMS") == nullptr;
  const bool fused_expert_finalize =
      std::getenv("GEVVA_DISABLE_VERIFIER_FUSED_EXPERT_FINALIZE") == nullptr;
  const bool fused_dense_gate_up = std::getenv("GEVVA_FUSED_TARGET_GATE_UP") != nullptr;
  const bool parallel_dense_gate_up = !fused_dense_gate_up && sessions >= 4 &&
      std::getenv("GEVVA_DISABLE_PARALLEL_DENSE_GATE_UP") == nullptr;
  const bool fused_direct_qkv_enabled =
      std::getenv("GEVVA_DISABLE_FUSED_DIRECT_QKV") == nullptr;
  const bool scalar_global_attention =
      std::getenv("GEVVA_SCALAR_GLOBAL_ATTENTION") != nullptr;
  const bool scalar_sliding_attention =
      std::getenv("GEVVA_SCALAR_SLIDING_ATTENTION") != nullptr;
  ...
  const bool aligned_direct_attention =
      std::getenv("GEVVA_ALIGNED_DIRECT_ATTENTION") != nullptr;
