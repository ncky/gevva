...
TargetPrefillBenchmark benchmark_target_prefill(
    ...
    const void* device_projected_images, bool kv_only) {
  constexpr int kHidden = 2816, kDense = 2112, kExperts = 128, kTop = 8;
  if (kv_only && benchmark) throw std::runtime_error("KV-only prefill is not a diagnostic benchmark");
  ...
  if (tokens < 1 || tokens > 4608 || prefix_tokens < 0 || total_tokens < 2 ||
      ...
      total_tokens > cache.maximum_context() || prefix_tokens < cache.shared_prefix_tokens())
    throw std::runtime_error("invalid target prefill geometry");
  const bool fused_expert_finalize =
      std::getenv("GEVVA_DISABLE_PREFILL_FUSED_EXPERT_FINALIZE") == nullptr;
  ...
  const bool direct_outputs = fp8_linears && std::getenv("GEVVA_PREFILL_COPY_OUTPUTS") == nullptr;
  const int projection_rows = direct_outputs ? fp8_linears->padded_output_rows(tokens) : tokens;
  // Causal attention for a fresh text prefix can share a padded plan: real
  // queries cannot attend to the trailing padded keys. Never pad image islands
  // or a continuation, whose bottom-right mask has different semantics.
  const bool bucket_text_prefix = kv_only && direct_outputs && !prefix_tokens &&
      !image_tokens && std::getenv("GEVVA_CUDNN_TEXT_PREFILL") &&
      !std::getenv("GEVVA_DECISION_EXACT_PREFIX_SHAPES");
  const int prefix_attention_rows = bucket_text_prefix ? (tokens + 15) & ~15 : tokens;
  ...
  auto* q = allocate(static_cast<std::size_t>(projection_rows) * 8192,
                     "cudaMalloc(prefill Q)");
  auto* k = allocate(static_cast<std::size_t>(projection_rows) * 2048,
                     "cudaMalloc(prefill K)");
  auto* v = allocate(static_cast<std::size_t>(projection_rows) * 2048,
  ...
  auto* attended = allocate(static_cast<std::size_t>(prefix_attention_rows) * 8192,
  ...
  auto* attention = allocate(static_cast<std::size_t>(projection_rows) * kHidden,
  ...
  auto* router_logits = allocate(static_cast<std::size_t>(projection_rows) * kExperts,
                                 "cudaMalloc(prefill router logits)");
  auto* dense_gate = allocate(static_cast<std::size_t>(projection_rows) * kDense,
                              "cudaMalloc(prefill dense gate)");
  auto* dense_up = allocate(static_cast<std::size_t>(projection_rows) * kDense,
  ...
  auto* dense_output = allocate(static_cast<std::size_t>(projection_rows) * kHidden,
  ...
  const std::size_t prefix_metadata_bytes = input_ids.size_bytes() +
      image_rows.size() * sizeof(int) + block_ids.size() * sizeof(std::int32_t);
  auto* d_prefix_metadata = static_cast<std::byte*>(allocate_bytes(
      prefix_metadata_bytes, "cudaMalloc(prefill metadata)"));
  auto* d_ids = reinterpret_cast<std::uint32_t*>(d_prefix_metadata);
  auto* d_image_rows = reinterpret_cast<int*>(d_prefix_metadata + input_ids.size_bytes());
  auto* d_block_ids = reinterpret_cast<std::int32_t*>(d_prefix_metadata +
      input_ids.size_bytes() + image_rows.size() * sizeof(int));
  ...
  if (scratch && context) {
    auto* staging = static_cast<std::byte*>(scratch->host_staging(prefix_metadata_bytes));
    std::memcpy(staging, input_ids.data(), input_ids.size_bytes());
    std::memcpy(staging + input_ids.size_bytes(), image_rows.data(), image_rows.size() * sizeof(int));
    std::memcpy(staging + input_ids.size_bytes() + image_rows.size() * sizeof(int),
                block_ids.data(), block_ids.size() * sizeof(std::int32_t));
    check(cudaMemcpyAsync(d_prefix_metadata, staging, prefix_metadata_bytes,
                          cudaMemcpyHostToDevice, context->stream()), "upload pinned prefill metadata");
  } else {
  ...
  auto project = [&](const __nv_bfloat16* matrix, int rows, int columns,
    ...
    if (fp8_linears &&
        fp8_linears->launch(matrix, rows, columns, source, destination, tokens,
                            project_stream, handle, true, workspace_index, direct_outputs))
      return;
  ...
  const bool prefill_concurrency = context && fp8_linears &&
      std::getenv("GEVVA_DISABLE_PREFILL_CONCURRENCY") == nullptr;
  const bool prefill_combined_qkv =
      std::getenv("GEVVA_DISABLE_PREFILL_COMBINED_QKV_TRANSFORM") == nullptr;
  const bool experimental_text_prefill = image_tokens == 0 &&
      std::getenv("GEVVA_CUDNN_TEXT_PREFILL") != nullptr;
  const auto* rope_table = std::getenv("GEVVA_PRECOMPUTED_ROPE")
  ...
  auto launch = [&] {
    ...
    for (int layer = 0; layer < layer_count; ++layer) {
      ...
      if (!fp8_linears || !fp8_linears->launch_unscaled(
              ...
              tokens, stream, handle, q_scale, 0, direct_outputs))
      ...
      if (!fp8_linears || !fp8_linears->launch_unscaled(
              ...
              handle, k_scale, concurrent_qkv ? 1 : 0, direct_outputs))
      ...
      if (!global &&
          (!fp8_linears || !fp8_linears->launch_unscaled(
              ...
              handle, v_scale, concurrent_qkv ? 2 : 0, direct_outputs)))
      ...
      if (bucket_text_prefix && !global && cudnn_text_plan_available(1, prefix_attention_rows)) {
        if (prefix_attention_rows > tokens) {
          const auto tail = static_cast<std::size_t>(prefix_attention_rows - tokens);
          check(cudaMemsetAsync(q + static_cast<std::size_t>(tokens) * q_width, 0,
                                tail * q_width * 2, stream), "zero padded prefix Q");
          check(cudaMemsetAsync(k + static_cast<std::size_t>(tokens) * kv_width, 0,
                                tail * kv_width * 2, stream), "zero padded prefix K");
          check(cudaMemsetAsync(v + static_cast<std::size_t>(tokens) * kv_width, 0,
                                tail * kv_width * 2, stream), "zero padded prefix V");
        }
        launch_cudnn_text_prefill(q, k, v, attended, 1, prefix_attention_rows, stream);
      } else for (int first = 0; first < tokens; first += layer_query_tile) {
        ...
        launch_prefill_attention_impl(
            ...
            experimental_text_prefill && !bucket_text_prefix);
      }
      Fp8OutputScale attention_scale{};
      if (!fp8_linears || !fp8_linears->launch_unscaled(
              w.o, kHidden, q_width, attended,
              attention, tokens, stream, handle, attention_scale, 0, direct_outputs))
      ...
      if (!fp8_linears || !fp8_linears->launch_unscaled(
              ...
              router_logits, tokens, stream, handle, router_scale, 0, direct_outputs))
  ...
  if (benchmark) {
    ...
    microseconds = isolated_event_benchmark(stream, launch, 0, 1);
  }
  ...
  const bool collect_host_state = !kv_only && (benchmark || !device_final_state);
...
std::vector<TargetPrefillBenchmark> benchmark_target_prefill_batch(
    ...
    bool decision_only, bool read_only_prefix, bool multimodal_prefix) {
  constexpr int kHidden = 2816, kDense = 2112, kExperts = 128, kTop = 8;
  const int sessions = static_cast<int>(items.size());
  if (sessions < 1 || sessions > (decision_only ? 256 : 8) || layer_count < 1 || layer_count > 30 ||
      !context || !scratch || !persistent_vocab || !fp8_linears ||
      persistent_experts.size() != static_cast<std::size_t>(layer_count))
    throw std::runtime_error("invalid target batch prefill resources");

  if (read_only_prefix && !decision_only) throw std::runtime_error("read-only prefill is for decisions only");
  ...
  for (int session = 0; session < sessions; ++session) {
    ...
    if (!item.cache || tokens < 1 || item.multimodal_types.size() !=
        ...
        total > item.cache->maximum_context() || item.prefix_tokens < item.cache->shared_prefix_tokens())
      throw std::runtime_error("invalid target batch prefill item");
    if (read_only_prefix) {
      if (item.prefix_tokens != items[0].prefix_tokens || item.cache != items[0].cache ||
          item.prefix_tokens > item.cache->resident_context())
        throw std::runtime_error("read-only questions must share one resident context");
    } else item.cache->ensure_context(total, context->stream());
    maximum_tokens = std::max(maximum_tokens, tokens);
    for (int i = 0; i < tokens; ++i) token_positions.push_back(item.prefix_tokens + i);
    ...
    if (item.final_state_row < 0 || item.final_state_row >= (decision_only ? sessions : 8))
  ...
  if (read_only_prefix && std::any_of(block_ids.begin(), block_ids.end(), [](int id) { return id >= 0; }))
    throw std::runtime_error("images must be in the shared prefix, not question suffixes");
  ...
  const bool uniform_qkv_transform = uniform_geometry &&
      std::getenv("GEVVA_DISABLE_UNIFORM_BATCH_QKV_TRANSFORM") == nullptr;
  bool uniform_attention = !read_only_prefix && uniform_geometry &&
      uniform_prefix + uniform_tokens <= 4608 &&
      std::getenv("GEVVA_DISABLE_UNIFORM_BATCH_PREFILL_ATTENTION") == nullptr;
  const bool fused_expert_finalize =
      std::getenv("GEVVA_DISABLE_PREFILL_FUSED_EXPERT_FINALIZE") == nullptr;
  for (int session = 1; session < sessions; ++session)
    uniform_attention &= items[session].prefix_tokens == uniform_prefix;

  const bool segmented_global = read_only_prefix && std::getenv("GEVVA_DECISION_COMBINED_GLOBAL") == nullptr;
  const bool segmented_sliding = read_only_prefix &&
      (std::getenv("GEVVA_DECISION_SEGMENTED_SLIDING") != nullptr ||
       (std::getenv("GEVVA_DECISION_COMBINED_SLIDING") == nullptr &&
        (!multimodal_prefix || (sessions >= 8 && uniform_prefix >= 256))));
  const bool segmented_any = segmented_global || segmented_sliding;
  const bool batched_attention = uniform_attention || read_only_prefix;
  if (read_only_prefix) {
    // Bound cuDNN plan diversity for ragged requests without adding model rows.
    if (experimental_text_prefill) maximum_tokens = (maximum_tokens + 31) & ~31;
    maximum_query_tile = maximum_tokens;
    maximum_key_tokens = std::max(maximum_key_tokens, ((uniform_prefix + 7) & ~7) + ((maximum_tokens + 7) & ~7));
  }
  const bool direct_outputs = std::getenv("GEVVA_PREFILL_COPY_OUTPUTS") == nullptr;
  const int projection_rows = direct_outputs ? fp8_linears->padded_output_rows(rows) : rows;
  scratch->reset();
  ...
  auto* k = allocate(static_cast<std::size_t>(projection_rows) * 2048);
  auto* v = allocate(static_cast<std::size_t>(projection_rows) * 2048);
  const std::size_t combined_elements = read_only_prefix
      ? static_cast<std::size_t>(sessions) * std::max(
          (segmented_global ? maximum_tokens : (((uniform_prefix + 7) & ~7) + ((maximum_tokens + 7) & ~7))) * 1024,
          (segmented_sliding ? maximum_tokens : std::min(uniform_prefix, 1024) + maximum_tokens) * 2048)
      : static_cast<std::size_t>(uniform_attention ? sessions : 1) * maximum_sliding_key_tokens * 2048;
  auto* combined_k = allocate(combined_elements);
  auto* combined_v = allocate(combined_elements);
  auto* padded_q = read_only_prefix ? allocate(static_cast<std::size_t>(sessions) * maximum_tokens * 8192) : nullptr;
  auto* global_accumulation = segmented_any ? static_cast<float*>(allocate_bytes(static_cast<std::size_t>(sessions) * maximum_tokens * 8192 * sizeof(float))) : nullptr;
  auto* padded_attended = read_only_prefix ? allocate(static_cast<std::size_t>(sessions) * maximum_tokens * 8192) : nullptr;
  auto* padded_blocks = read_only_prefix ? static_cast<std::int32_t*>(allocate_bytes(sessions * maximum_tokens * sizeof(int))) : nullptr;
  auto* combined_key_views = read_only_prefix ? static_cast<__nv_bfloat16**>(allocate_bytes(sessions * sizeof(void*))) : nullptr;
  auto* combined_value_views = read_only_prefix ? static_cast<__nv_bfloat16**>(allocate_bytes(sessions * sizeof(void*))) : nullptr;
  ...
  auto* attention_pointers = allocate(
      (static_cast<std::size_t>(batched_attention ? sessions + (segmented_any ? 1 : 0) : 1) * 96 *
           sizeof(void*) +
  ...
  const std::size_t row_offsets_offset = reserve_metadata(row_offsets.size() * sizeof(int), 4);
  const std::size_t token_positions_offset = reserve_metadata(token_positions.size() * sizeof(int), 4);
  ...
  auto* d_row_offsets = reinterpret_cast<int*>(d_metadata + row_offsets_offset);
  auto* d_token_positions = reinterpret_cast<int*>(d_metadata + token_positions_offset);
  ...
  std::memcpy(host_metadata + row_offsets_offset, row_offsets.data(), row_offsets.size() * sizeof(int));
  std::memcpy(host_metadata + token_positions_offset, token_positions.data(), token_positions.size() * sizeof(int));
  if (read_only_prefix) check(cudaMemsetAsync(padded_blocks, 0xff, sessions * maximum_tokens * sizeof(int), stream), "set decision text masks");
  if (decision_only) check(cudaMemsetAsync(selected_tokens, 0xff, sessions * sizeof(int), stream), "initialize unused decision token selection");
  ...
  auto launch = [&] {
    ...
    for (int layer = 0; layer < layer_count; ++layer) {
      ...
      if (!fp8_linears->launch_unscaled(
              ...
              q_scale, 0, direct_outputs))
        project(w.q, q_width, kHidden, normalized, q);
      if (!fp8_linears->launch_unscaled(
              w.k, kv_width, kHidden, normalized, k, rows,
              context->auxiliary_stream(), handle, k_scale, 1, direct_outputs))
      ...
      if (!global && !fp8_linears->launch_unscaled(
              ...
              context->tertiary_stream(), handle, v_scale, 2, direct_outputs))
      ...
      if (read_only_prefix) {
        if (global)
          transform_qkv_ragged_kernel<512, 2, true><<<rows * 18, 512, 0, stream>>>(
              q, k, v, w.q_norm, w.k_norm, rows, d_token_positions, 1,
              q_scale.activation_rows, q_scale.weight, k_scale.activation_rows, k_scale.weight,
              v_scale.activation_rows, v_scale.weight, -1, rope_table);
        else
          transform_qkv_ragged_kernel<256, 8, false><<<rows * 24, 256, 0, stream>>>(
              q, k, v, w.q_norm, w.k_norm, rows, d_token_positions, 1,
              q_scale.activation_rows, q_scale.weight, k_scale.activation_rows, k_scale.weight,
              v_scale.activation_rows, v_scale.weight, -1, rope_table);
      } else if (uniform_qkv_transform) {
        if (global)
          transform_qkv_ragged_kernel<512, 2, true>
      ...
      for (int session = 0; !read_only_prefix && session < sessions; ++session) {
        const auto& item = items[session];
        const int row = row_offsets[session];
        const int tokens = row_offsets[session + 1] - row;
        const int total = item.prefix_tokens + tokens;
      ...
      if (read_only_prefix) {
        // An aligned score leading dimension avoids cuBLAS's slow align1
        // global-attention kernels. Extra keys are future-masked zeros.
        const bool segmented = global ? segmented_global : segmented_sliding;
        const int past_tokens = global ? uniform_prefix : std::min(uniform_prefix, 1024);
        const int key_tokens = (global || segmented) ? (((past_tokens + 7) & ~7) + ((maximum_tokens + 7) & ~7))
                                     : std::min(uniform_prefix, 1024) + maximum_tokens;
        const auto& prefix_cache = items[0].cache->layer(layer);
        const std::size_t q_vectors = static_cast<std::size_t>(sessions) * maximum_tokens * q_width / 8;
        const int packed_prefix = segmented ? 0 : uniform_prefix;
        const int packed_keys = segmented ? maximum_tokens : key_tokens;
        const std::size_t kv_vectors = static_cast<std::size_t>(sessions) * packed_keys * kv_width / 8;
        const int q_blocks = static_cast<int>(std::min<std::size_t>(65535, (q_vectors + 255) / 256));
        const int kv_blocks = static_cast<int>(std::min<std::size_t>(65535, (kv_vectors + 255) / 256));
        pack_decision_queries_kernel<<<q_blocks, 256, 0, stream>>>(
            reinterpret_cast<const uint4*>(q), reinterpret_cast<uint4*>(padded_q),
            d_row_offsets, sessions, maximum_tokens, q_width / 8, false);
        pack_decision_keys_kernel<<<kv_blocks, 256, 0, stream>>>(
            reinterpret_cast<const uint4*>(k), reinterpret_cast<const uint4*>(v),
            reinterpret_cast<const uint4*>(prefix_cache.keys), reinterpret_cast<const uint4*>(prefix_cache.values),
            reinterpret_cast<uint4*>(combined_k), reinterpret_cast<uint4*>(combined_v),
            combined_key_views, combined_value_views, d_row_offsets, sessions, maximum_tokens, packed_prefix, global, packed_keys);
        if (segmented) {
          const int first_slot = global ? 0 : ((uniform_prefix - past_tokens) & 1023);
          const auto* past_keys = reinterpret_cast<const __nv_bfloat16*>(prefix_cache.keys) + first_slot * kv_width;
          const auto* past_values = reinterpret_cast<const __nv_bfloat16*>(prefix_cache.values) + first_slot * kv_width;
          if (global)
            launch_segmented_attention<512, 2>(handle, padded_q, past_keys, past_values,
                combined_k, combined_v, scores, global_accumulation, padded_attended, attention_pointers,
                sessions, maximum_tokens, past_tokens, key_tokens, false, stream);
          else
            launch_segmented_attention<256, 8>(handle, padded_q, past_keys, past_values,
                combined_k, combined_v, scores, global_accumulation, padded_attended, attention_pointers,
                sessions, maximum_tokens, past_tokens, key_tokens, true, stream);
        }
        else if (!global && experimental_text_prefill && cudnn_text_plan_available(sessions, maximum_tokens, key_tokens))
          launch_cudnn_text_prefill(padded_q, combined_k, combined_v,
                                    padded_attended, sessions, maximum_tokens, stream, key_tokens);
        else if (global)
          launch_uniform_batch_prefill_attention<512, 2, true>(
              handle, padded_q, combined_k, combined_v, combined_key_views, combined_value_views,
              combined_k, combined_v, scores, padded_blocks, padded_attended, attention_pointers,
              sessions, maximum_tokens, key_tokens, uniform_prefix, key_tokens, stream);
        else
          launch_uniform_batch_prefill_attention<256, 8, false>(
              ...
              sessions, maximum_tokens, key_tokens, uniform_prefix, key_tokens, stream);
        pack_decision_queries_kernel<<<q_blocks, 256, 0, stream>>>(
            reinterpret_cast<const uint4*>(padded_attended), reinterpret_cast<uint4*>(attended),
            d_row_offsets, sessions, maximum_tokens, q_width / 8, true);
      }
      ...
      if (uniform_attention) {
        ...
        if (!global && experimental_text_prefill && cudnn_text_plan_available(sessions, uniform_tokens, key_tokens))
      ...
      if (!fp8_linears->launch_unscaled(
              ...
              attention_scale, 0, direct_outputs))
      ...
      if (!fp8_linears->launch_unscaled(
              ...
              stream, handle, router_scale, 0, direct_outputs))
      ...
      if (!fp8_linears->launch_unscaled(
              ...
              dense_stream, handle, down_scale, 1, direct_outputs))
  ...
  const float microseconds = isolated_event_benchmark(stream, launch, 0, 1);
  auto* host_tokens = static_cast<int*>(
      scratch->host_staging(sessions * sizeof(int)));
  if (!decision_only) check(cudaMemcpyAsync(host_tokens, selected_tokens,
  ...
  for (int session = 0; session < sessions; ++session) {
    ...
    result.selected_token = decision_only ? 0 : host_tokens[session];
